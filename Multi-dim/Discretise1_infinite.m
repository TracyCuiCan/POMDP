...
% t=1:T-1
c=C(length(C));
for t=2:1000 % run for at most 1000 iterations
    for i=1:point_num
        Q=trans(i,:)*V(t-1,:)'; 
        VC(t,i)=c+Q;
        for j=1:n
    ...
    if max(abs(V(t,:)-V(t-1,:)))< 0.001 % check if the differnece is less than epsilon
        break
    end
end
T = size(V,1);
...
upper = zeros(1,T);
for t=2:T
    lower(t)=(find(G(t,:)==3,1)-1)/M;
    upper(t)=(find(G(t,:)==1,1)-1)/M;
end
toc
end
